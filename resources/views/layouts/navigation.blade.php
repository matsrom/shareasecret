<nav x-data="{ open: false }" class="shadow relative">
    <!-- Primary Navigation Menu -->
    <div class="mx-auto">
        <div class="flex justify-between h-24 bg-blue-700 py-6 px-6 md:px-20">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @if (Auth::check())
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                class="inline-flex items-center px-3 py-2 border border-transparent leading-4 font-medium rounded-md text-white hover:text-slate-200 focus:outline-none transition ease-in-out duration-150">
        ...
        <!-- Backdrop -->
        <div :class="{ 'block': open, 'hidden': !open }" class="fixed inset-0 top-24 bg-black bg-opacity-50 z-10"></div>
        <!-- Responsive Navigation Menu -->
        <div :class="{ 'block': open, 'hidden': !open }"
            class="absolute top-24 left-0 right-0 hidden sm:hidden bg-white z-20">
            @if (Auth::check())
